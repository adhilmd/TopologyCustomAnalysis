...
p <- ggplot(data=disfreq, aes(x=DistanceGroup, y=perc, fill=collisiontype)) + geom_bar(stat="identity", position=position_dodge()) + xlab("Origin to Transcript Collision Distance") + ylab("Percentage of Gene numbers") 
fname = paste(outdir,"ReplicationTranscriptionCollision.jpeg",sep="/")
ggsave(p, file = fname, height = 6, width = 8, dpi = 300)
fname = paste(outdir,"ReplicationTranscriptionCollision.txt",sep="/")
...
for (i in 1:length(sbdg)){
  ...
  p <- ggplot(data=fdat, aes(x=Distance, y=Percentage, fill=Collision)) + geom_bar(stat="identity", position=position_dodge()) + xlab("Origin to Transcript Collision Distance") + ylab(yname)
  fname = paste(outdir,"/",stags[i],"_RepTrans_Collision_Percentage.jpeg",sep="")
  ggsave(p, file = fname, height = 6, width = 8, dpi = 300)
  fname = paste(outdir,"/",stags[i],"_RepTrans_Collision_Percentage.txt",sep="")
  write.table(file = fname,fdat,sep="\t",row.names=FALSE,col.names=TRUE,quote=FALSE)
}
