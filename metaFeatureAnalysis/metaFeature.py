...
def argparser():
    ...
    parser.add_argument('-avgt','--avgtype', type=str, dest='avgtype', help="Ratio averaging type mean or median (Default = median)", action = 'store', default ="median",choices=["median","mean"])
...
def featuremerge(featurechunk,bdgfile,beddf,basesmerging,strand,flanking,flankingbases):
    ...
    lcn = mdf.shape[1]
    mdf.columns = [i for i in range(0, lcn)]
    mdf = mdf.drop(mdf.columns[[lcn-2, lcn-3, lcn-4]], axis = 1) 
    if (flanking == 'yes'):
...
def cfstats(df):
    cf95_lo = df.scorer.mean() - df.scorer.sem() * 1.96
    cf95_hi = df.scorer.mean() + df.scorer.sem() * 1.96
    return cf95_lo, cf95_hi

def main(featuredf,procs,bdgfile,bedfile,strand,basesmerging,flanking,flankingbases,avgtype,samplename,sitems):
    print (os.getcwd())
    ...
    if (flanking == 'yes'):
        ...
        findat = bfn
        del bfn
    aggfn = findat.groupby('value', as_index=False).agg({"scorer": avgtype, "Dens":"sum"})
    aggfncf = pd.DataFrame(findat.groupby('value').apply(cfstats))
    aggfncf.columns = ['CF']
    aggfncf[['CF_low', 'CF_high']] = pd.DataFrame(aggfncf['CF'].tolist(), index=aggfncf.index)
    aggfn = aggfn.merge(aggfncf,on="value")
    #aggfn = findat.copy()
    aggfn['samplename'] = samplename
    aggfn = aggfn[["value","scorer","Dens","samplename","CF_low", "CF_high"]]
    nowtime = time.time()
...
if __name__ == "__main__":
    ...
    if (args.subtype != None):
        ...
        for items in subtypes:
            ...
            result = main(itypes,args.procs,args.bdgfile,args.bedfile,args.strand,args.basesmerging,args.flanking,args.flankingbases,args.avgtype,args.samplename,items)
        ...
        items = "NA"
        print (args)
        result = main(featuredf,args.procs,args.bdgfile,args.bedfile,args.strand,args.basesmerging,args.flanking,args.flankingbases,args.avgtype,args.samplename,items)
